(ns imo_dangerous_goods.main
  "Kotodama WASM entrypoint for the imo_dangerous_goods clean-room actor (L4) — Clojure port.
  ...
  facts (`imo_dangerous_goods.<Entity>/<field>`); `*store*` is the in-memory
  materialization used by the contract test and by the WASM runtime before a
  live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "imo_dangerous_goods")
(def tier "L4")
...
(def entity-specs
  [{:entity "TradeItem" :plural "tradeitems" :id-prefix "imodange_tra"
    :fields [:gtin :description :brand]           :required [:gtin :description]
    :coerce {}                       :refs {}}
   {:entity "Event"     :plural "events"     :id-prefix "imodange_eve"
    :fields [:itemId :bizStep :disposition :recordedAt] :required [:bizStep :disposition]
    :coerce {}                       :refs {}}
   {:entity "Document"  :plural "documents" :id-prefix "imodange_doc"
    :fields [:type :senderId :receiverId :status]  :required [:type :status]
    :coerce {}                       :refs {}}
   {:entity "Container" :plural "containers" :id-prefix "imodange_con"
    :fields [:bicCode :type :status]           :required [:bicCode :type]
    :coerce {}                       :refs {}}
   {:entity "Party"     :plural "parties"   :id-prefix "imodange_par"
    :fields [:gln :name :role]               :required [:gln :name]
    :coerce {}                       :refs {}}
   {:entity "Shipment"  :plural "shipments" :id-prefix "imodange_shi"
...
(defn emit-facts
  "EAVT facts for one record: {\"imo_dangerous_goods.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "imo_dangerous_goods-compat" :tier tier :entities entities} 200])
...
(defn start! [] :imo_dangerous_goods-compat/ready)
